...
Credits:
    //made by emin - github.com/eminbaritci/turkstar
...
GitHub: https://github.com/eminbaritci/turkstar
...
$(document).ready(() => {
  ...
  app.titleChanger(['emin', 'emin', 'emin', 'emin', 'emin', '=', 'turkstar.xyz', 'sarki adi: KUSO GVKI - Cinnamon (feat. Apfel) (splash! Mag Premiere)']);
...
$.getJSON(ipgeolocation, (data) => {
  writeLine(['taraniyor...', "giris yapiliyor... <span style='font-size: 14px; color: #06d;'>[unknown]</span>..."], 30, () => {
    ...
    writeLine([`giris tamamlandi! <span style='font-size: 14px; color: #0f0;'>[basarili]</span>`, `hosgeldin, <i style='color: #0f0'>${ip}</i>! bu arada, seni ${country} gibi bir yerden gormek guzel!`], 30, 500, () => {
      if (app.skippedIntro) return;

      clearCursor();

      writeLine([`<i style='color: #F62459'>turkstar.xyz $$$</i>`], 120, 500, () => {
        timeouts.push(
          setTimeout(() => {
            if (app.skippedIntro) return;
